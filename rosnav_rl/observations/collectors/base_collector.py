...
class CollectionError(Exception):
    ...
    def __init__(self, collector_name: str, message: str, original_exception: Optional[Exception] = None) -> None:
    ...
    def safe_preprocess(self, msg: MessageType) -> ProcessedObservationType:
        ...
        try:
            if not self.validate_message(msg):
                rospy.logwarn_once(
                    f"[{self.__class__.__name__}] Message validation failed for {type(msg)}"
                )
            ...
            if self.fallback_value is not None:
                rospy.logwarn(
                    f"[{self.__class__.__name__}] Using fallback value after error: {e}"
                )
